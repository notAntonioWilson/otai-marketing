      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10">
        <div className="text-center max-w-2xl mx-auto mb-8 md:mb-10">
          ...
          <p className="text-slate-400 text-base md:text-xl leading-[1.7]" style={{fontFamily:"var(--font-body)"}}>
            3+ years of organic social media marketing. These numbers are pulled directly from platform analytics and client dashboards — no inflated claims, no paid reach.
          </p>
        </div>

        {/* Stats grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-10 md:mb-12 reveal">
          {stats.map(s => (
            <div key={s.label} className="gradient-border p-5 text-center">
              <div className="text-2xl md:text-3xl font-bold mb-2" style={{color:s.color,fontFamily:"var(--font-display)"}}>{s.value}</div>
        ...
        {/* Credibility note */}
        <div className="gradient-border px-6 md:px-8 py-5 md:py-6 text-center mb-8 md:mb-10 reveal">
          <p className="text-slate-300 text-sm md:text-base leading-[1.75]" style={{fontFamily:"var(--font-body)"}}>
            These results come from real accounts across contractors, trades, and service businesses. Some accounts are no longer accessible due to client transitions — the screenshots below are the direct proof.{" "}
